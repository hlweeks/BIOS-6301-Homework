...
```
The gender coefficient seems to have the largest effect.
```
...
FALSE.  Finally multiply this result by two. (4 points)

```{r}
testGender <- mod.c["genderM","t value"]
2*pt(testGender, df=7, lower.tail=FALSE)
```
...
```
```
The two methods result in the same answer of 33.8.
```
...
```
```
You can see that the "Residual standard error" output is 2.197, which matches our calculation above.  
This could also be extracted from summary(mod) using the following:
```
```{r}
summary(mod)$sigma
```
...
```{r}
(tUnequal <- t.test(women$los, men$los))
(tEqual <- t.test(women$los, men$los, var.equal=TRUE))

#Extract p-values
tUnequal$p.value
tEqual$p.value
summary(mod)$coefficients["genderM","Pr(>|t|)"]
```
```
The p-value from the model summary is 0.02243214.

The first t.test (without setting var.equal=TRUE) produced a p-value of 0.02204748,  
which is near but not equal to the model summary.

The second t.test (with setting var.equal=TRUE) produced a p-value of .02243214,  
which exactly matches the model summary.
```
